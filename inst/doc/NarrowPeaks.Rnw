...
Next, the candidate binding site regions are extracted using the R/Bioconductor package {\CSAR} \cite{Muino2011}. CSAR predictions are contiguous genomic regions separated by a maximum allowed of \Robject{gap} base pairs, and score enrichment values greater than \Robject{thr}. 

<<write score binaries from a WIG file and extract candidate regions of enrichment>>=  
library(NarrowPeaks)
data("NarrowPeaks-dataset")
head(wigfile_test)
writeLines(wigfile_test, con="wigfile.wig")
wigScores <- wig2CSARScore(wigfilename="wigfile.wig", nbchr = 1, 
chrle=c(30427671), thr=1.0, gap=30)
print(wigScores$infoscores$filenames)
@

Candite regions are stored in a \Robject{GRanges} object (see Bioconductor package \Rpackage{GenomicRanges}).

<<candidate enriched regions in GRanges object>>=
head(wigScores$candidates)
@

If {\CSAR} \cite{Muino2011} is used first to analyze ChIP-seq data, from its results we can obtain the false discovery rate (FDR) for a given threshold. For example, for the complete experiment described in \cite{Kaufmann2010}, \Robject{thr = 10.81} corresponds to FDR = 0.01 and \Robject{thr = 6.78} corresponds to FDR = 0.1. \\
...
shortpeaksP0 <- narrowpeaks(inputReg=wigScores$candidates, 
...
shortpeaksP3 <- narrowpeaks(inputReg=wigScores$candidates, 
scoresInfo=wigScores$infoscores, lmin=0, nbf=150, rpenalty=0,
 nderiv=0, npcomp=2, pv=80, pmaxscor=3.0, ms=0)
head(shortpeaksP3$broadPeaks)
head(shortpeaksP3$narrowPeaks)
shortpeaksP100 <- narrowpeaks(inputReg=wigScores$candidates, 
scoresInfo=wigScores$infoscores, lmin=0, nbf=150, rpenalty=0, 
...
shortpeaksP90 <- narrowpeaks(inputReg=wigScores$candidates,
 scoresInfo=wigScores$infoscores, lmin=0, nbf=150, rpenalty=0, 
nderiv=0, npcomp=2, pv=80, pmaxscor=90, ms=0)
shortpeaksP90ms20 <- narrowpeaks(inputReg=wigScores$candidates,
 scoresInfo=wigScores$infoscores, lmin=0, nbf=150, rpenalty=0,
...
library(rtracklayer)
export.bedGraph(object=wigScores$candidates, con="CSAR.bed")
