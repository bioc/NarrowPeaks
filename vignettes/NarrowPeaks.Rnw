%\VignetteIndexEntry{NarrowPeaks Vignette. Shape-based splitting and trimming ChIP-Seq peaks using functional Principal Components}
%\VignetteDepends{}
...
\title{An Introduction to the \Biocpkg{NarrowPeaks} Package:\\ Narrowing Down Transcription Factor Binding Site Candidates from ChIP-Seq using Functional PCA}
\date{Created: January, 2013. Last modified: June, 2014. Compiled: \today}
...
Comprehensive ChIP-Seq data analyses are carried out by many software tools \cite{Bailey2013}. 
